...
#  HAVE_PETSC         - PETSc found and usable
...
# Setting
#  PETSC_ROOT
# searches in file "petscvariables" for PETSC_DIR and PETSC_ARCH
# 
...
function (petsc_get_version)
  if (EXISTS "${PETSC_INCLUDE_DIR}/include/petscversion.h")
    file (STRINGS "${PETSC_INCLUDE_DIR}/include/petscversion.h" vstrings REGEX "#define PETSC_VERSION_(RELEASE|MAJOR|MINOR|SUBMINOR|PATCH) ")
    ...
    message (SEND_ERROR "PETSC_INCLUDE_DIR can not be used, ${PETSC_INCLUDE_DIR}/include/petscversion.h does not exist")
  endif ()
endfunction ()

#try to read from in petscvariables file found in PETSC_ROOT first
if(NOT PETSC_INCLUDE_DIR OR NOT PETSC_ARCH)
  if( EXISTS "${PETSC_ROOT}/conf/petscvariables")
    file(STRINGS "${PETSC_ROOT}/conf/petscvariables" PETSC_ARCH REGEX "PETSC_ARCH=") 
    string (REGEX REPLACE " *= *" ";" fields ${PETSC_ARCH}) # break line into two fields
    list (GET fields 1 PETSC_ARCH)
    file(STRINGS "${PETSC_ROOT}/conf/petscvariables" PETSC_INCLUDE_DIR REGEX "PETSC_DIR=") 
    string (REGEX REPLACE " *= *" ";" fields ${PETSC_INCLUDE_DIR}) # break line into two fields
    list (GET fields 1 PETSC_INCLUDE_DIR)
  endif()
endif(NOT PETSC_INCLUDE_DIR OR NOT PETSC_ARCH)

find_path (PETSC_INCLUDE_DIR include/petsc.h
  HINTS $ENV{PETSC_DIR}
  PATHS ${PETSC_ROOT} /usr/lib/petscdir /usr/include/petsc   
  PATH_SUFFIXES "3.1" "3.2" "3.4.2" "3.0.0" "2.3.3" "2.3.2" #Debian
  DOC "PETSc Directory")


find_program (MAKE_EXECUTABLE NAMES make gmake)

if(PETSC_INCLUDE_DIR AND NOT PETSC_ARCH)
  set (_petsc_arches
...
endif(PETSC_INCLUDE_DIR AND NOT PETSC_ARCH)
...
# new-style (>= 3.0.0)
if (EXISTS "${PETSC_INCLUDE_DIR}/${PETSC_ARCH}/include/petscconf.h")   # > 2.3.3
  set (petsc_conf_rules "${PETSC_INCLUDE_DIR}/conf/rules")
  set (petsc_conf_variables "${PETSC_INCLUDE_DIR}/conf/variables")
elseif (EXISTS "${PETSC_INCLUDE_DIR}/bmake/${PETSC_ARCH}/petscconf.h") # <= 2.3.3
  set (petsc_conf_rules "${PETSC_INCLUDE_DIR}/bmake/common/rules")
  set (petsc_conf_variables "${PETSC_INCLUDE_DIR}/bmake/common/variables")
elseif (PETSC_INCLUDE_DIR)
  message (SEND_ERROR "The pair PETSC_INCLUDE_DIR=${PETSC_INCLUDE_DIR} PETSC_ARCH=${PETSC_ARCH} does not specify a valid PETSc installation")
endif ()
...
if (petsc_conf_rules AND petsc_conf_variables AND NOT petsc_config_current)
  ...
  set (ENV{PETSC_DIR} "${PETSC_INCLUDE_DIR}")
...
")
    ...
    if (${${runs}})
      set (PETSC_EXECUTABLE_RUNS "YES" CACHE BOOL
        "Can the system successfully run a PETSc executable?  This variable can be manually set to \"YES\" to force CMake to accept a given PETSc configuration, but this will almost always result in a broken build.  If you change PETSC_ROOT, PETSC_ARCH, or PETSC_CURRENT you would have to reset this variable." FORCE)
    endif (${${runs}})
  endmacro (PETSC_TEST_RUNS)

  find_path (PETSC_INCLUDE_DIR petscts.h HINTS "${PETSC_INCLUDE_DIR}" PATH_SUFFIXES include NO_DEFAULT_PATH)
  find_path (PETSC_INCLUDE_CONF petscconf.h HINTS "${PETSC_INCLUDE_DIR}" PATH_SUFFIXES "${PETSC_ARCH}/include" "bmake/${PETSC_ARCH}" NO_DEFAULT_PATH)
...
find_package_handle_standard_args (PETSc
  "PETSc could not be found.  Be sure to set PETSC_ROOT and PETSC_ARCH."
...
if(PETSC_FOUND)
  foreach(dir "${PETSC_INCLUDE_DIRS}")
