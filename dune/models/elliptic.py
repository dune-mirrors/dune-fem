    ...
    if comm.rank == 0:
        ...
        if not os.path.isfile(os.path.join(compilePath, name + ".so")):
            ...
            if model.coefficients:
                ...
                writer.emit('    return k;')
                writer.emit('  };')
                writer.closeFunction()

                writer.openFunction('void setCoefficient', targs=['std::size_t i'], args=['Model &model', 'pybind11::handle o'])
                ...
                writer.typedef('std::function< void( Model &model, pybind11::handle ) >', 'Dispatch')
                writer.emit('std::array< Dispatch, sizeof...( i ) > dispatch = {{ Dispatch( setCoefficient< i > )... }};')
                writer.emit('')
                writer.emit('return [ dispatch ] ( ModelWrapper &model, pybind11::handle coeff, pybind11::handle o ) {')
                writer.emit('    std::size_t k = coeff.attr("number").template cast<int>();')
