...
namespace Dune
{
  ...
  namespace Fem
  {
    ...
    public:
      ...
      SizeType size () const { return mapper1_.size() + mapper2_.size(); }

      /** \copydoc Dune::DofMapper::contains(const int codim) const */
      bool contains ( const int codim ) const { return ( mapper1_.contains( codim ) || mapper2_.contains( codim ) ); }

      /** \copydoc Dune::DofMapper::consecutive const */
      bool consecutive () const { return mapper1_.consecutive() && mapper2_.consecutive(); }

      /** \copydoc Dune::DofMapper::fixedDataSize(int codim) const */
      bool fixedDataSize ( int codim ) const { return mapper1_.fixedDataSize( codim ) && mapper2_.fixedDataSize( codim ); }


      // mappings

      /** \copydoc Dune::DofMapper::numDofs(const ElementType &element) const */
      unsigned int numDofs ( const ElementType &element ) const { return mapper1_.numDofs( element ) + mapper2_.numDofs( element ); }

      /** \copydoc Dune::DofMapper::maxNumDofs const */
      unsigned int maxNumDofs () const { return mapper1_.maxNumDofs() + mapper2_.maxNumDofs(); }

      /** \copydoc Dune::DofMapper::numEntityDofs(const Entity &entity) const */
      template< class Entity >
      unsigned int numEntityDofs ( const Entity &entity ) const { return mapper1_.numEntityDofs( entity ) + mapper2_.numEntityDofs( entity ); }
      ...
      int numBlocks () const { return mapper1_.numBlocks() + mapper2_.numBlocks(); }
      ...
      {
        assert( (hole >= 0) && hole < numberOfHoles( block ) );
        const int numBlock1 = mapper1_.numBlocks();
        if( block - numBlock1 < 0 )
      ...
      bool compress () { resize(); return true; }

      template< class Entity >
      void insertEntity ( const Entity & ) { resize(); }

      template< class Entity >
      void removeEntity ( const Entity & ) {}

      template< class IOStream >
      void read ( IOStream &in ) { resize(); }
