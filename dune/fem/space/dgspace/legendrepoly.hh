...
namespace Dune
{
  ...
  {  
    ...
    public: 
      static const LegendrePoly& instance() 
      {
        static LegendrePoly legPoly;
        return legPoly;
      }
      ...
      double evaluate(const int num, const double x) const
      {  
      ...
      double jacobian(const int num, const double x) const
      { 
        assert(0<=num && num<maxPol);
        double phi=0.0;
        if (num>=1)
      ...
      double hessian(const int num, const double x) const
      { 
        assert(0<=num && num<maxPol);
        double phi=0.0;    
