...
namespace Dune {
  ...
  namespace Fem {
    ...
    /**Define a lumping quadrature for all geometries. Note, however,
     * that this may not make sense for anything else than simplices
     * or maybe hexagonal grids. For simplicial meshes the quadrature
     * formula is exact on linear polynomials and hence the quadrature
     * error is quadratic in the mesh-size. A mass-matrix assembled
     * with the caching quadrature will be diagonal in the context of
     * Lagrange spaces. Generally, it is a bad idea to use
     * mass-lumping for anything else than linear (or maybe bilinear)
     * finite elements.
     *
     * There are probably much more efficient ways to perform
     * mass-lumping. This "quadrature" approach is convenient,
     * however, as it can simply be plugged into existing code by
     * replacing the quadrature rules.
     */
    ...
    /**Non-caching version of the LumpingQuadrature. */
    template<class GridPart, int codim>
    using ElementLumpingQuadrature = ElementQuadrature<GridPart, codim, LumpingQuadratureTraits>;

    /**Caching version of the LumpingQuadrature. */
    template<class GridPart, int codim>
    using CachingLumpingQuadrature = CachingQuadrature<GridPart, codim, LumpingQuadratureTraits>;
