#ifndef LAPLACEOPERATOR_HH
#define LAPLACEOPERATOR_HH
...
{
  typedef LaplaceOperator< DiscreteFunction, LinearOperator > ThisType;
...
public:
  explicit LaplaceOperator ( const DiscreteFunctionSpaceType &dfSpace )
...
template< class DiscreteFunction, class LinearOperator >
void LaplaceOperator< DiscreteFunction, LinearOperator >::assemble ()
{
  ...
  Dune::DynamicVector< typename DiscreteFunctionSpaceType::JacobianRangeType > dvalues;
  ...
  for( const auto &entity : elements( dfSpace_.gridPart(), Dune::Partitions::interiorBorder ) )
  {
    ...
    dvalues.resize( numBasisFunctions );
    ...
    for( const auto qp : QuadratureType( entity, localMatrix.order() ) )
    {
      ...
      std::for_each( dvalues.begin(), dvalues.end(), [ weight ] ( auto &a ) { a *= weight; } );

      // update system matrix
      localMatrix.axpy( qp, values, dvalues );
    }
  }
}

#endif // #ifndef LAPLACEOPERATOR_HH
