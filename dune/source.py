...
class BaseModel:
    ...
    def setCoef(self, sourceWriter, modelClass='Model', wrapperClass='ModelWrapper'):
        ...
        sourceWriter.openFunction('void setCoefficient', targs=['std::size_t i'], args=[ modelClass + ' &model', 'pybind11::object o'])
        ...
        sourceWriter.typedef('std::function< void( ' + modelClass + ' &model, pybind11::object ) >', 'Dispatch')
        sourceWriter.emit('std::array< Dispatch, sizeof...( i ) > dispatch = {{ Dispatch( setCoefficient< i > )... }};')
        sourceWriter.emit('')
        sourceWriter.emit('return [ dispatch ] ( ' + wrapperClass + ' &model, pybind11::object coeff, pybind11::object o ) {')
        sourceWriter.emit('    std::size_t k = coeff.attr("number").template cast<int>();')
