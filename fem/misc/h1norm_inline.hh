namespace Dune
{
  ...
  inline H1Norm< GridPartImp > :: H1Norm ( const GridPartType &gridPart )
  : gridPart_( gridPart )
  {
  }
  ...
  template< class GridPartImp >
  inline H1Norm< GridPartImp > :: H1Norm ( const ThisType &other )
  ...
  H1Norm< GridPartImp > :: norm ( const DiscreteFunctionType &u ) const
  {
    ...
    for( GridIteratorType it = gridPart_.template begin< 0 >(); it != end; ++it )
    {
      ...
      for( unsigned int qp = 0; qp < numQuadraturePoints; ++qp )
      {
        ...
        JacobianRangeType Duqp;
        ulocal.jacobian( quadrature[ qp ], Duqp );

        for( unsigned int i = 0; i < dimRange; ++i )
          sum += factor * (Duqp[ i ] * Duqp[ i ]);
      }
    }
  ...
  H1Norm< GridPartImp > :: distance ( const UDiscreteFunctionType &u,
                                      const VDiscreteFunctionType &v ) const
  {
    ...
    typedef typename UDiscreteFunctionType :: JacobianRangeType JacobianRangeType;
    ...
    for( GridIteratorType it = gridPart_.template begin< 0 >(); it != end; ++it )
    {
      ...
      for( unsigned int qp = 0; qp < numQuadraturePoints; ++qp )
      {
        ...
        JacobianRangeType Duqp, Dvqp;
        ulocal.jacobian( quadrature[ qp ], Duqp );
        vlocal.jacobian( quadrature[ qp ], Dvqp );
        Duqp -= Dvqp;
