...
DIRK::DIRK(const Communicator &comm, int num_of_stages, int order, Function &f,
...
DIRK::LinearOperator::LinearOperator(const Communicator &comm, Function &f,
				     const int &dim, 
...
//class DIRK3
static const double delta = 1.0/2.0 + sqrt(3.0)/6.0;
static const double DIRK3_A[] =
  {delta, 0.0,
   1.0-2.0*delta, delta
  };
static const double DIRK3_b[] = 
  {(0.5-delta)/(1.0-2.0*delta), (0.5-delta)/(1.0-2.0*delta)};
static const double DIRK3_c[] = 
  {delta, 1.0-delta};

DIRK3::DIRK3(const Communicator &comm, Function &f) :
...
ImplicitEuler::ImplicitEuler(const Communicator &comm, Function &f) :
...
Gauss2::Gauss2(const Communicator &comm, Function &f) :
