...
inline 
Matrix::Matrix(int n) : n(n), m(n), data(new double[n*n])
{
  assert(data);
  dset(n*m, 0.0, data, 1);
}
...
inline 
Matrix::Matrix(const Matrix &A) : n(A.n), m(A.m), data(new double[A.n*A.m])
{
  assert(data);
...
inline 
Matrix::Matrix(int n, int m, const double *a) : n(n), m(m), 
						data(new double[n*m])
{
  assert(data);
...
inline 
Matrix::Matrix(int n, const double *a) : n(n), m(n), data(new double[n*n])
{
  assert(data);
  cblas_dcopy(n*m, a, 1, data, 1);
}
...
inline
void Matrix::operator()(const double *u, double *f, int i)
{
  switch (i){
  case 0: // function
...
inline 
int Matrix::dim_of_argument(int i) const
{
  switch (i){
  case 0: return m;
  case 1: return n*m;
...
inline
int Matrix::dim_of_value(int i) const
{
  switch (i){
  case 0: return n;
  case 1: return n*m;
