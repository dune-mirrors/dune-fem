// $Id: fvector.hh 4850 2007-02-19 14:13:41Z christi $
...
//    typedef typename BlockIterator::DifferenceType DifferenceType;
  ...
  */
  ...
  class FieldVector
   ...
   template <class T, class K, int i> 
   struct TempMeta
   {
      static void assign(T& a, const K& k)
      {
        a[i] = k;
        TempMeta<T,K,i-1>::assign(a,k);
      }
      static void add(const T& org, T& res)
      {
        res[i] += org[i];
        TempMeta<T,K,i-1>::add(org,res);
      }
      static void addK(T& a, const K& k)
      {
        a[i] += k;
        TempMeta<T,K,i-1>::addK(a,k);
      }
      
      static void sub(const T& org, T& res)
      {
        res[i] -= org[i];
        TempMeta<T,K,i-1>::sub(org,res);
      }
      static void subK(T& a, const K& k)
      {
        a[i] -= k;
        TempMeta<T,K,i-1>::subK(a,k);
      }
      static K skp(const T& a, const T& b)
      {
        return TempMeta<T,K,i-1>::skp(a,b) + a[i]*b[i];
      } 
      static void axpy(const T& b, const K& k, T& a)
      {
        a[i] += b[i]*k;
        TempMeta<T,K,i-1>::axpy(b,k,a);
      } 
      static void scale(T& a, const K& k)
      {
        a[i] *= k;
        TempMeta<T,K,i-1>::scale(a,k);
      }
   };
  
   template <class T, class K> 
   struct TempMeta<T,K,0>
   {
      static void assign(T& a, const K& k)
      {
        a[0] = k;
  ...
  */
  ...
  typedef FieldVector<K,SIZE> ThisType;
	...
	{
    TempMeta<ThisType,K,size-1>::assign(*this,k);
    ...
    /*
            for (size_type i=0; i<SIZE; i++)
                p[i] += y.p[i];
                */
    TempMeta<ThisType,K,size-1>::add(y,*this);
	  return *this;
	}
	...
	FieldVector& operator-= (const FieldVector& y)
	{
    TempMeta<ThisType,K,size-1>::sub(y,*this);
    //        for (size_type i=0; i<SIZE; i++)
    //            p[i] -= y.p[i];
	  return *this;
	}
    ...
    /*
            for (size_type i=0; i<SIZE; i++)
                p[i] += k;
                */
    TempMeta<ThisType,K,size-1>::addK(*this,k);
	  return *this;
	}
	...
	FieldVector& operator-= (const K& k)
	{
    TempMeta<ThisType,K,size-1>::subK(*this,k);
    //        for (size_type i=0; i<SIZE; i++)
    //            p[i] -= k;
    return *this;
	}
    ...
    //        for (size_type i=0; i<SIZE; i++)
    //            p[i] *= k;
    TempMeta<ThisType,K,size-1>::scale(*this,k);
	  return *this;
	}
	...
	{
    K k1 = 1.0/k; 
    TempMeta<ThisType,K,size-1>::scale(*this,k1);
    //      for (size_type i=0; i<SIZE; i++)
    //          p[i] /= k;
	  return *this;
	}
	...
	{
#ifndef DUNE_EXPRESSIONTEMPLATES
    TempMeta<ThisType,K,size-1>::axpy(y,a,*this);
    //        for (size_type i=0; i<SIZE; i++)
    //            p[i] += a*y.p[i];
#else
    *this += a*y;
	...
	{
    return TempMeta<ThisType,K,size-1>::skp(*this,y);
    //        K result = 0;
  ...
  double two_norm () const
	{
    return std::sqrt(TempMeta<ThisType,K,size-1>::skp(*this,*this));
    /*
          double result = 0;
          for (int i=0; i<size; i++)
